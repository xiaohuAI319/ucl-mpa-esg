...
// Constants
const STORAGE_KEY = 'ucl_mpa_workspace_v6_cute';
const CONFIG_KEY = 'ucl_mpa_gpt_config_v3';
...
const INITIAL_MESSAGES: Message[] = [
  { 
    id: 'intro', 
    role: 'assistant', 
    content: "Hi friend! 🐻 I'm your UCL MPA study buddy. \n\nUpload your notes (DOCX/TXT/MD) on the left, and I'll help you ace your ESG analysis!",
    timestamp: Date.now() 
  }
];
...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedData) setFolders(JSON.parse(savedData).folders || []);
    
    // Merge saved config with default to ensure new fields (like new providers) exist
    if (savedConfig) {
      const parsed = JSON.parse(savedConfig);
      // Simple migration check: if structure is old (no 'providers'), reset or try to migrate
      if (!parsed.providers) {
        // Old config format, ignore or manual migrate could go here. 
        // For safety, we use default but maybe try to salvage openai key? 
        // For now, let's just use defaults to prevent crash.
        setSettings(DEFAULT_SETTINGS);
      } else {
        setSettings(prev => ({
          ...prev,
          ...parsed,
          providers: { ...prev.providers, ...parsed.providers }
        }));
      }
    }
  }, []);
  ...
  const handleAddFolder = () => {
    ...
    setFolders([...folders, { id: Date.now().toString(), name: newFolderName, files: [] }]);
    setNewFolderName('');
  };
      ...
      <div className="w-full max-w-7xl h-full md:h-[95vh] bg-white/60 backdrop-blur-xl rounded-[2.5rem] shadow-card border-4 border-white flex flex-col md:flex-row overflow-hidden relative">
               ...
               {folders.map(folder => (
                 <div key={folder.id} className="bg-white rounded-2xl p-4 border-2 border-slate-50 shadow-sm hover:shadow-md hover:border-indigo-50 transition-all group">
                   ...
                   <label className="flex items-center justify-center gap-2 w-full py-2 border-2 border-dashed border-indigo-100 rounded-xl text-xs font-bold text-indigo-300 hover:text-indigo-500 hover:border-indigo-300 hover:bg-indigo-50 cursor-pointer transition-all">
                     ...
                     <span>Add Notes</span>
                     <input type="file" multiple accept=".txt,.md,.json,.csv,.docx" className="hidden" onChange={(e) => handleFileUpload(folder.id, e.target.files)} />
                   </label>
                 </div>
               ))}
        ...
        <main className={`flex-1 flex flex-col bg-[#FDF6E3]/30 relative ${activeTab === 'chat' ? 'flex' : 'hidden md:flex'}`}>
          ...
          <div className="px-6 py-4 flex justify-between items-center border-b-2 border-white/50 bg-white/40 backdrop-blur-md sticky top-0 z-10">
            ...
            {/* Toggle Search (Only relevant for Gemini usually, but nice to have in UI) */}
